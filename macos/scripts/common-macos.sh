...
recorded_injector_command_matches() {
  local pid="$1"
...
recorded_injector_process_matches() {
  ...
  recorded_injector_command_matches "$pid" "$expected_node" "$expected_injector" "$expected_port" || return 1
  ...
  if ! recorded_injector_process_matches "$pid" "$saved_start" "$saved_node" "$saved_injector" "$saved_port"; then
    ...
    case "$saved_start" in
      ?\ [0-9]*/[0-9]*\ [0-9][0-9]:[0-9][0-9]:[0-9][0-9]\ [0-9][0-9][0-9][0-9])
        if recorded_injector_command_matches "$pid" "$saved_node" "$saved_injector" "$saved_port"; then
          saved_start="$(process_started_at "$pid")"
        else
