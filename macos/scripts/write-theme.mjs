...
const sourceUrlValue = hasValue("source-url") ? valueFor("source-url") : "";
const sourceProvider = hasValue("source-provider")
  ? validateChoice(valueFor("source-provider"), "source-provider", ["Wikimedia Commons", "Dream Skin Community"])
  : "Wikimedia Commons";
let sourceUrl = "";
if (sourceUrlValue) {
  const parsed = new URL(sourceUrlValue);
  const validWikimedia = sourceProvider === "Wikimedia Commons" && parsed.hostname === "commons.wikimedia.org";
  const validCommunity = sourceProvider === "Dream Skin Community" && parsed.hostname === "github.com" &&
    parsed.pathname.startsWith("/iStig/Codex-Skin-Clinet/issues/");
  if (parsed.protocol !== "https:" || (!validWikimedia && !validCommunity) || parsed.href.length > 500)
    throw new Error("source-url does not match its trusted source provider.");
  sourceUrl = parsed.href;
}
const sourceAuthor = hasValue("source-author")
  ? validateText(valueFor("source-author"), "source-author", 160, "") : "";
const sourceLicense = hasValue("source-license")
  ? validateText(valueFor("source-license"), "source-license", 120, "") : "";
