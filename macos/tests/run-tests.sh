...
if /usr/bin/grep -F -q 'pgrep -x ChatGPT' "$ROOT/scripts/status-dream-skin-macos.sh" ||
  ! /usr/bin/grep -F -q 'CFBundleIdentifier' "$ROOT/scripts/status-dream-skin-macos.sh" ||
  ! /usr/bin/grep -F -q '"com.openai.codex"' "$ROOT/scripts/status-dream-skin-macos.sh"; then
  printf 'Status must identify the Codex bundle instead of matching every ChatGPT process.\n' >&2
  exit 1
fi
if ! /usr/bin/grep -F -q 'LC_ALL=C /bin/ps' "$ROOT/scripts/common-macos.sh" ||
  ! /usr/bin/grep -F -q 'LC_ALL=C /bin/ps' "$ROOT/scripts/status-dream-skin-macos.sh"; then
  printf 'Injector start-time identity must be independent of the user locale.\n' >&2
  exit 1
fi
if ! /usr/bin/grep -F -q 'process.exit(124)' "$ROOT/scripts/injector.mjs"; then
  printf 'One-shot injector commands must retain a process-level deadline.\n' >&2
  exit 1
fi
...
ONLINE_HOME="$TMP/online-save-home"
ONLINE_STATE="$ONLINE_HOME/Library/Application Support/CodexDreamSkinStudio"
/bin/mkdir -p "$ONLINE_STATE/theme"
/usr/bin/printf '%s\n' 'keep-current' > "$ONLINE_STATE/theme/sentinel"
for online_id in 101 102; do
  /usr/bin/env HOME="$ONLINE_HOME" NODE="$NODE" \
    "$ROOT/scripts/load-image-theme-macos.sh" \
    --file "$ROOT/assets/portal-hero.png" --name "Online $online_id" \
    --source-url "https://commons.wikimedia.org/wiki/File:Online-$online_id.jpg" \
    --save-only --theme-id "online-$online_id" >/dev/null
done
/usr/bin/env HOME="$ONLINE_HOME" NODE="$NODE" \
  "$ROOT/scripts/load-image-theme-macos.sh" \
  --file "$ROOT/assets/portal-hero.png" --name 'Duplicate' \
  --source-url 'https://commons.wikimedia.org/wiki/File:Duplicate.jpg' \
  --save-only --theme-id online-101 >/dev/null
/usr/bin/grep -F -q 'keep-current' "$ONLINE_STATE/theme/sentinel"
[ -f "$ONLINE_STATE/themes/online-101/theme.json" ]
[ -f "$ONLINE_STATE/themes/online-102/theme.json" ]
/usr/bin/grep -F -q 'File:Online-101.jpg' "$ONLINE_STATE/themes/online-101/theme.json"
if /usr/bin/grep -F -q 'File:Duplicate.jpg' "$ONLINE_STATE/themes/online-101/theme.json"; then
  printf 'Save-only online import overwrote an existing stable theme id.\n' >&2
  exit 1
fi
[ -z "$(/usr/bin/find "$ONLINE_STATE/themes" -maxdepth 1 -name '.theme-import.*' -print -quit)" ]
...
"$NODE" "$ROOT/scripts/write-theme.mjs" custom --output-dir "$TMP/theme" \
  ...
  --source-url 'https://commons.wikimedia.org/wiki/File:Test.jpg' \
  --source-author '测试作者' --source-license 'CC BY-SA 4.0' >/dev/null
...
"$NODE" -e '
  ...
  if (theme.source?.provider !== "Wikimedia Commons") process.exit(1);
  if (theme.source?.author !== "测试作者" || theme.source?.license !== "CC BY-SA 4.0") process.exit(1);
  if (theme.source?.url !== "https://commons.wikimedia.org/wiki/File:Test.jpg") process.exit(1);
...
/bin/mkdir -p "$TMP/community-theme"
/bin/cp "$ROOT/assets/portal-hero.png" "$TMP/community-theme/background.png"
"$NODE" "$ROOT/scripts/write-theme.mjs" custom --output-dir "$TMP/community-theme" \
  --image background.png --name '社区主题' \
  --source-provider 'Dream Skin Community' \
  --source-url 'https://github.com/iStig/Codex-Skin-Clinet/issues/123' \
  --source-author '社区作者' --source-license 'CC BY 4.0' >/dev/null
"$NODE" -e '
  const theme = JSON.parse(require("fs").readFileSync(process.argv[1], "utf8"));
  if (theme.source?.provider !== "Dream Skin Community") process.exit(1);
  if (theme.source?.url !== "https://github.com/iStig/Codex-Skin-Clinet/issues/123") process.exit(1);
' "$TMP/community-theme/theme.json"

# Large source JPEGs are valid up to 50 MB, but the prepared theme image must
# be normalized below the 16 MB injector limit instead of taking the fast-copy path.
LARGE_JPEG_HOME="$TMP/large-jpeg-home"
LARGE_JPEG_SOURCE="$TMP/large-source.jpg"
/usr/bin/sips -s format jpeg "$ROOT/assets/portal-hero.png" --out "$LARGE_JPEG_SOURCE" >/dev/null
/bin/dd if=/dev/zero of="$LARGE_JPEG_SOURCE" bs=1 count=1 seek=16777216 conv=notrunc 2>/dev/null
/usr/bin/env HOME="$LARGE_JPEG_HOME" NODE="$NODE" \
  "$ROOT/scripts/load-image-theme-macos.sh" --file "$LARGE_JPEG_SOURCE" \
  --name '大图测试' --no-apply >/dev/null
LARGE_PREPARED="$LARGE_JPEG_HOME/Library/Application Support/CodexDreamSkinStudio/theme/background.jpg"
[ -s "$LARGE_PREPARED" ]
[ "$(/usr/bin/stat -f '%z' "$LARGE_PREPARED")" -le 16777216 ]
...
assert_write_theme_rejected source-host --source-url 'https://example.com/wallpaper.jpg'
assert_write_theme_rejected source-author-control --source-author $'unsafe\nauthor'
