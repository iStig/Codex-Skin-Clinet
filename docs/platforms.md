...
- macOS 在线图库将 Wikimedia 与 Dream Skin 社区作为两个独立来源；本地主题页也将本地/内置壁纸和网络下载壁纸分区展示。社区投稿通过 `iStig/Codex-Skin-Clinet` 的 GitHub Issue 上传附件并声明 CC0、CC BY 4.0 或 CC BY-SA 4.0，审核通过后写入该仓库的 `community/catalog.json`。客户端不保存 GitHub Token，不提供匿名直传接口，也不需要自建存储服务器；GitHub 属于零成本、小规模方案而非无限 CDN，具体审核与迁移边界见 `community/README.md`。
